...
class TempCategory:
    ...
    def __init__(self, category, indent):
        self.category = (
            category  # versioninfo, visgroups, world, solid, dispinfo, etc...
        )
        self.indent = indent
        self.data = (
            []
        )  # Everything inside the curly brackets other than child categories (ex: "skyname" "sky_dust")
        self.children = (
            []
        )  # List of all children categories (ex: side, dispinfo, editor, etc...)
        self.current_child = None  # Used when going into nested children (ex: solid -> side -> dispinfo -> Normals)
        self.dic = (
            {}
        )  # This is where all the data is stored when it's cleaned, used when creating VMF class
    ...
    def add_line(self, line, indent):
        ...
        diff = (
            indent - self.indent
        )  # Finding out how far into the nested children we need to go
        c = self
    ...
    def clean_up(self):
        ...
        for i in self.data:
            clean = re.findall(
                r"\"(.*?)\"", i
            )  # We remove the double quotes and separate (example line: "id" "2688")
            self.dic[clean[0]] = num(
                clean[1]
            )  # The values, IF possible are turned into either ints or floats
    ...
    with open(file, "r") as vmf:
        ...
        for line in vmf.readlines()[1:]:
            if "}" in line:
                ...
                if (
                    indent == 0
                ):  # If indent is not 0 we know it's a child category and not a main category
                    extracted.append(t)
                continue
            ...
            if "{" in line:
                if (
                    ...
                    t = TempCategory(
                        previous_line, indent
                    )  # This is a main category (not a child category)
                indent += 1
                continue
            ...
            if '"' in line:
                t.add_line(
                    line, indent
                )  # ALL lines with data have double quotes in them

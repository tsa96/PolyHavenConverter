...
def triangulate_displacement(
    vmf: VMF, group: List[Solid], base_triangle: Solid = None, resolution=1, height=2
):
    if base_triangle is None:
        base_triangle = SolidGenerator.displacement_triangle(
            Vertex(0, 0, 0), 1, 1, height
        )
    ...
    l_extremities = [
        [(False, True), None, (False, False), (True, False)],
        [(False, True), (True, True), (False, False), None],
    ]
    r_extremities = [
        [(False, True), (True, True), None, (True, False)],
        ...
        if disp:
            ...
            for sy in range(0, disp.matrix_size_fix, resolution):
                ...
                for sx in range(0, disp.matrix_size_fix, resolution):
                    ...
                    left_tri.move(
                        sx * size.x / resolution, sy * -size.y / resolution, 0
                    )
                    ...
                    i = 0
                    for x, y, vert in matrix.inv_rect(
                        sx, sy, 2 * resolution, 2 * resolution, resolution
                    ):

...
class Convert:
    ...
    def string_to_3x_vertex(string: str) -> List[Vertex, Vertex, Vertex]:
        ...
        return [
            Vertex(clean[0], clean[1], clean[2]),
...
class Vertex(
    Common
):  # Vertex has to be above the Solid class (see: set_pos_vertex function)
    """
    ...
    def __init__(self, x=0, y=0, z=0):
        ...
        self.normal = (
            0  # Vertices are represented differently in the VMF depending on the class
        )
    ...
    def similar(self, other, accuracy=0.001) -> bool:
        ...
        """
        return (
            (abs(self.x - other.x) < accuracy)
            and (abs(self.y - other.y) < accuracy)
            and (abs(self.z - other.z) < accuracy)
        )
    ...
    def rotate_z(self, center: Vertex, angle):
        ...
        new_x = (
            center.x
            + (self.x - center.x) * math.cos(a)
            - (self.y - center.y) * math.sin(a)
        )
        new_y = (
            center.y
            + (self.x - center.x) * math.sin(a)
            + (self.y - center.y) * math.cos(a)
        )
    ...
    def rotate_y(self, center: Vertex, angle):
        ...
        new_x = (
            center.x
            + (self.x - center.x) * math.cos(a)
            - (self.z - center.z) * math.sin(a)
        )
        new_z = (
            center.z
            + (self.x - center.x) * math.sin(a)
            + (self.z - center.z) * math.cos(a)
        )
...
class Solid(Common):
    ...
    def get_axis_extremity(
        self, x: bool = None, y: bool = None, z: bool = None
    ) -> Vertex:
        """
        ...
        for iz in range(z):
            for iy in range(y):
                for ix in range(x):
                    s2 = s.copy()
                    s2.move(
                        ix * move_amount.x, -iy * move_amount.y, -iz * move_amount.z
                    )
                    li.append(s2)
...
class DispInfo(Common):
    ...
    def __init__(
        self, dic: dict = None, children: list = None, parent_side: Side = None
    ):
        dic, children = self._dic_and_children(dic, children)
...
class Vector(Common):
    ...
    def vectors_from_side(cls, side: Side) -> Tuple[Vector, Vector]:
        return cls.vector_from_2_vertices(
            side.plane[1], side.plane[0]
...
class SolidGenerator:
    ...
    def cube(vertex: Vertex, w, h, l, center=False, dev=0) -> Solid:
        ...
        x, y, z = vertex.export()
        f1 = Side(
            dic={"plane": f"({x + w} {y} {z + l}) ({x + w} {y} {z}) ({x} {y} {z})"}
        )
        f2 = Side(
            dic={
                "plane": f"({x + w} {y + h} {z}) ({x + w} {y + h} {z + l}) ({x} {y + h} {z + l})"
            }
        )
        f3 = Side(
            dic={"plane": f"({x} {y} {z}) ({x} {y + h} {z}) ({x} {y + h} {z + l})"}
        )
        f4 = Side(
            dic={
                "plane": f"({x + w} {y + h} {z}) ({x + w} {y} {z}) ({x + w} {y} {z + l})"
            }
        )
        f5 = Side(
            dic={
                "plane": f"({x} {y} {z + l}) ({x} {y + h} {z + l}) ({x + w} {y + h} {z + l})"
            }
        )
    ...
    def displacement_triangle(vertex: Vertex, w, h, l, dev=0) -> Solid:
        ...
        f3 = Side(
            dic={
                "plane": f"({x} {y} {z + l}) ({x} {y + h} {z + l}) ({x + w} {y} {z + l})"
            }
        )
        f4 = Side(
            dic={"plane": f"({x + w} {y} {z + l}) ({x + w} {y} {z}) ({x} {y} {z})"}
        ...
        f5 = Side(
            dic={"plane": f"({x} {y + h} {z + l}) ({x} {y + h} {z}) ({x + w} {y} {z})"}
        )
    ...
    @staticmethod
    def room(
        vertex: Vertex, w, h, l, thick: int = 64, dev=0
    ) -> List[Solid, Solid, Solid, Solid, Solid, Solid]:
        """
    ...
    def surf_ramp(
        ...
        side_cut=32,
        center=False,
        ramp_texture="cs_italy/cobble02",
        top_texture="cs_italy/plasterwall02a",
        side_texture="cs_italy/plasterwall02a",
    ) -> Solid:
        """
        ...
        f1 = Side(
            dic={
                "plane": f"({x - ww} {y} {z}) ({x - w} {y + side_cut} {z}) ({x - top_cut} {y + hh} {z})"
            }
        )
        f2 = Side(
            dic={
                "plane": f"({x - ww} {y + side_cut} {z - ll}) ({x - ww} {y} {z - ll}) ({x + ww} {y} {z - ll})"
            }
        )
        f3 = Side(
            dic={
                "plane": f"({x - ww} {y} {z - ll}) ({x - ww} {y + side_cut} {z - ll}) "
                f"({x - ww} {y + side_cut} {z})"
            }
        )
        f4 = Side(
            dic={
                "plane": f"({x + ww} {y + side_cut} {z - ll}) ({x + ww} {y} {z - ll}) ({x + ww} {y} {z})"
            }
        )
        f5 = Side(
            dic={
                "plane": f"({x - top_cut} {y + hh} {z - ll}) ({x + top_cut} {y + hh} {z - ll}) "
                f"({x + top_cut} {y + hh} {z})"
            }
        )
        f6 = Side(
            dic={
                "plane": f"({x + ww} {y} {z - ll}) ({x - ww} {y} {z - ll}) ({x - ww} {y} {z})"
            }
        )
        f7 = Side(
            dic={
                "plane": f"({x - ww} {y + side_cut} {z - ll}) ({x - top_cut} {y + hh} {z - ll}) "
                f"({x - top_cut} {y + hh} {z})"
            }
        )
        f8 = Side(
            dic={
                "plane": f"({x + top_cut} {y + hh} {z - ll}) ({x + ww} {y + side_cut} {z - ll}) "
                f"({x + ww} {y + side_cut} {z})"
            }
        )
...
class EntityGenerator:
    ...
    @staticmethod
    def info_overlay(
        origin: Vertex, texture: str, angle: Vertex = Vertex(), *sides: Side
    ) -> InfoOverlay:
        """
...
class VMF:
    ...
    def get_solids(
        self, include_hidden=False, include_solid_entities=True
    ) -> List[Solid, ...]:
        """
        Gets all the solids
    ...
    def get_entities(
        self, include_hidden=False, include_solid_entities=False
    ) -> List[Entity, ...]:
        """
    ...
    def get_solids_and_entities(self, include_hidden=False):
        ...
        """
        return self.get_solids(include_hidden, False) + self.get_entities(
            include_hidden, True
        )
    ...
    def mark_vertex(
        self, vertex: Vertex, size: int = 32, dev: int = 1, visgroup: str = None
    ):
        """
    ...
    def _nest_export(self, category):
        ...
        ):  # Some classes export None (ex: Hidden class export_children function)
    ...
    def _get_export_size(
        self, category
    ):  # Same concept as _nest_export just without writing to file

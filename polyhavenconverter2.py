...
ASSET_DIR = "./assets/polyhaven/"
TEXTURE_META = "./textures.yml"  # cheeky bit of yaml for once (‾◡◝)
DEFAULT_PARALLAX_DEPTH = 0.01
API_URL = "https://api.polyhaven.com"

# Nothing fancy, just various PolyHaven categories and tags to surfaceprops
# according to https://developer.valvesoftware.com/wiki/Material_surface_properties,
# and pick the first match.
SURFACE_PROP_MAP = {
    "asphalt": "Concrete",
    "bark": "Wood_LowDensity",
    "brick": "Brick",
    "cobblestone": "Rock",
    "concrete": "Concrete",
    "fabric": "Carpet",
...
"""
PolyHaven's OpenAPI docs are pretty terrible. So, for reference, the /assets/ endpoint returns data like
...
def load_asset_list():
    textures = {}
    # with open(TEXTURE_META, "r") as f:
    #     textures = json.load(f)


def infer_surface_prop(categories: list[str]):
    for category in categories:
        if category in SURFACE_PROP_MAP:
            return SURFACE_PROP_MAP[category]
    return None


def update_asset_list():
    fetched_assets = requests.get(f"{API_URL}/assets/", params={"type": "textures"})
    for asset_id, asset_data in fetched_assets.json().items():
        if asset_id in Textures:
            continue

        fetched_files = requests.get(API_URL + f"/files/{asset_id}").json()
        files = {}

        for map_key, map_name in PH_MAPS.items():
            file = fetched_files[map_key][RESOLUTION][FORMAT]
            files[map_name] = {"url": file["url"], "md5": file["md5"]}
        ...
        if surface_prop := infer_surface_prop(
            asset_data["categories"] + asset_data["tags"]
        ):
            Textures[asset_id]["surfaceProp"] = surface_prop

    with open(TEXTURE_META, "w") as f:
        f.write(yaml.safe_dump(Textures, sort_keys=False))
...
#                         print(
#                             f"  File {out_name} already exists and passes hash check, skipping")
#                     else:
#                         print(
#                             f"  File {out_name} already exists but failed hash check, redownloading")
...
class VMT:
    ...
    def __init__(self, file_name, tex_name, params=None):
        ...
        self.out_dir: str = os.getcwd() + "/" + ASSET_DIR
        self.max_len: int = 0
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        self.max_len = len(
            max(
                list(self.params.keys()) + ["basetexture", "bumpmap", "mraotexture"],
                key=len,
            )
        )
        ...
        self.__write_param("basetexture", ASSET_DIR + self.tex_name + "_diffuse")
        self.__write_param("bumpmap", ASSET_DIR + self.tex_name + "_hormal")
        self.__write_param("mraotexture", ASSET_DIR + self.tex_name + "_mrao")
...
# parser.add_argument(
#     "exec", help="the part of the program to run [download, convert, vmf, all]"
# )
# parser.add_argument(
#     "--forceconvert",
#     help="perform VTF conversion even if file exists",
#     action="store_true",
# )
